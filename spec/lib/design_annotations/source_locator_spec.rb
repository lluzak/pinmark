...
RSpec.describe DesignAnnotations::SourceLocator do
  it "returns relative source path with line number for a known class" do
    location = described_class.for(DesignAnnotations::Tracker)

    # Path is relative to Rails.root (the dummy app), so the engine source
    # appears as ../../lib/design_annotations/tracker.rb:N or similar.
