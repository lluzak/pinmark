...
RSpec.describe DesignAnnotations::Mcp::Queue do
  let(:queue_path) { Rails.root.join("tmp", "design_annotations", "queue_spec_#{SecureRandom.hex(4)}.json") }
  ...
  describe "#default_path" do
    it "returns a path under tmp/design_annotations" do
      expect(described_class.default_path.to_s).to end_with("tmp/design_annotations/queue.json")
    end
  end
  ...
  describe "atomic write" do
    ...
    it "round-trips text_excerpt through write/read" do
      ...
      entry = queue.read.fetch("annotations").first
      expect(entry["node_id"]).to eq("da-7")
