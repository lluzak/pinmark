...
RSpec.describe "Pinmark::Annotations", type: :request do
  let(:queue_path) { Rails.root.join("tmp/pinmark/queue.json") }
  ...
  context "POST /dev/pinmark in development with valid payload" do
    before do
    ...
    it "persists the annotation node id under node_id on the stored entry" do
      stored = JSON.parse(queue_path.read)
      entry = stored["annotations"].first
      expect(entry["node_id"]).to eq("pm-1")
      expect(entry.keys).not_to include("pm_id")
    end
  ...
  context "GET /dev/pinmark in development" do
    before { allow(Rails.env).to receive(:development?).and_return(true) }
  ...
  context "DELETE /dev/pinmark/:id in development" do
    before { allow(Rails.env).to receive(:development?).and_return(true) }
    ...
    it "removes the matching entry from the queue" do
      ...
      delete "/dev/pinmark/annotations/x1"
  ...
  context "outside of development env" do
    ...
    it "returns 404 on DELETE" do
      delete "/dev/pinmark/annotations/anything"
      expect(response).to have_http_status(:not_found)
    end
  end
end
